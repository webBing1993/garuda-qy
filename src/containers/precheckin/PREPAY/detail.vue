    ...
    computed: {
      ...
      payInfo() {
        if (this.payMode === 2) {
          ...
          return `<div class="cell-body">` +
            `<p><span class="cell-key3">已确认</span><span class="cell-right " style="font-size: 14px;color: #DF4A4A;">后付/挂账/公付等</span></p>` +
            `</div>`
        }
      },
      payMode(){
    ...
    methods: {
      ...
      getCellBodyPMS(){
        return this.detail.payinfo ? `<div class="cell-body">` +
          ...
          `<p><span class="cell-key">PMS预付：</span><span class="cell-value">${this.cashHandling(this.detail.payinfo.pms_pay)}</span></p>` +
          `</div>` : null;
      },
      getCellFooter(){
