<template>
  <article>
    ...
    <scroller lock-x v-show="currenttab ==='待确认'">
      <section class="tobeconfirmed" :class="{'checklist':batch}">
        <checker v-model="batchlist"
          ...
          <checker-item v-for="(item,index) in tbc" :key="index" :value="item.orderId">
            <Group class="group" @click.native="orderClick(item.orderId)">
              <cell>
                <span>订单号：</span>
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      orderClick(path){
        !this.batch ? ( this.batchlist = [], this.goto('/pms/prepay/' + path) ) : null
      }
    }
  }
</script>
